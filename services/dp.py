    ...
    for sig in candidates:
            ...
            changelog.append(
                {
                    ...
                    "student_id": sid,
                    "slot": (
                        f'{new_session["start_time"]}'
                        f'–{new_session["end_time"]}'
                    ),
                    "reason": (
                        f"New {sig.get('severity')} "
                        f"{sig.get('signal_type')} signal surfaced"
                        + (
                            " (moved up from tomorrow); "
                            if was_deferred
                            else "; "
                        )
                        + f"a free slot was available, "
                        f"so {sid} was scheduled today."
                    ),
                }
            )
            ...
            decisions.append(
                {
                    ...
                    "tradeoff": (
                        f'Both {sid} (incoming) and '
                        f'{incumbent["student_id"]} '
                        f'(scheduled '
                        f'{incumbent["start_time"]}–'
                        f'{incumbent["end_time"]}) '
                        f"are Critical, and today's "
                        f"{max_slots} slots are full. "
                        f"Only one can be seen today. "
                        f"Keeping {incumbent['student_id']} "
                        f"means {sid} waits until tomorrow; "
                        f"swapping means "
                        f"{incumbent['student_id']} is deferred "
                        f"instead. You decide."
                    ),
                    "options": [
                        {
                            "action": "keep_incumbent",
        ...
        if cand_pri < inc_pri:
            ...
            deferred.append(
                _make_deferred(
                    incumbent,
                    f"Bumped to tomorrow: {sid} surfaced a "
                    f"higher-priority "
                    f"{sig.get('severity')} "
                    f"{sig.get('signal_type')} concern and "
                    f"today's {max_slots} slots were full.",
                )
            )
            ...
            changelog.append(
                {
                    ...
                    "reason": (
                        ...
                        f"took the "
                        f'{new_session["start_time"]} slot '
                        f'from {incumbent["student_id"]} '
                        f"({incumbent.get('severity')}), "
                        f"who was the lowest-priority student "
                        f"scheduled and is now deferred "
                        f"to tomorrow."
                    ),
                }
            )
            ...
            deferred.append(
                _make_deferred(
                    sig,
                    f"Today's {max_slots} slots are full "
                    f"with equal-or-higher priority students; "
                    f"{sid} is deferred to tomorrow.",
                )
            )
            ...
            changelog.append(
                {
                    ...
                    "reason": (
                        ...
                        f"for {sid}, but all {max_slots} slots "
                        f"hold equal-or-higher priority students. "
                        f"Deferred to tomorrow."
                    ),
                }
            )
